...
$cek=mysqli_num_rows(mysqli_query($conn, "SELECT * FROM gallery_komentarfoto WHERE UserID='$user_id' AND FotoID='$foto_id' AND KomentarID='$komen_id'"));
if ($cek > 0) {
   $delete=mysqli_query($conn, "DELETE FROM gallery_komentarfoto WHERE KomentarID='$komen_id'");
