        ...
        <div class="col-7">
            <div class="card">
                <div class="card-body">
                    ...
                    <?php
                    $user = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM gallery_user WHERE UserID='{$_SESSION['user_id']}'"));
                    if (isset($_POST['editprofile'])) {
                        ...
                        if (isset($username) && isset($email)) {
                            if ($username == $user['Username'] && $email == $user['Email'] && $alamat == $user['Alamat']) {
                                $ubah = mysqli_query($conn, "UPDATE gallery_user SET NamaLengkap='$nama' WHERE UserID='$_SESSION[user_id]'");
                                $session = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM gallery_user WHERE UserID='$_SESSION[user_id]'"));
                                if ($ubah) {
                                ...
                                $ubah = mysqli_query($conn, "UPDATE gallery_user SET Alamat='$alamat' WHERE UserID='$_SESSION[user_id]'");
                                if ($ubah) {
                                ...
                                $ubah = mysqli_query($conn, "UPDATE gallery_user SET Email='$email' WHERE UserID='$_SESSION[user_id]'");
                                ...
                                $ubah = mysqli_query($conn, "UPDATE gallery_user SET Username='$username' WHERE UserID='$_SESSION[user_id]'");
                                $session = mysqli_fetch_array(mysqli_query($conn, "SELECT * FROM gallery_user WHERE UserID='$_SESSION[user_id]'"));
                                if ($ubah) {
                                    $_SESSION['userid'] = $session['UserID'];
                                    $_SESSION['username'] = $session['Username'];
                        ...
                        if ($password != $user['Password']) {
                            $ubah = mysqli_query($conn, "UPDATE gallery_user SET Password='$password' WHERE UserID='$_SESSION[user_id]'");
                            if ($ubah) {
